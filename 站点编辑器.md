...
- `1.0.0` 暂时未提供手动增加站点用于 `阅读模式` 的功能，`聚焦模式` 则可手动添加；
- `1.0.0` 暂时未提供全局的站点编辑器；

### 收录了哪些网址：
- 目前收录了超过 120 个域名；
- 可以通过这个 [链接](http://ojec5ddd5.bkt.clouddn.com/website_list.json) 查看 `SiteEditor` 收录的站点，可使用 `ctrl + F` 或 `control + F` 查询主域名；
...
### 如何新增：
- 阅读模式；  
  目前 `1.0.0` 暂时未开放 `新增阅读模式` 的功能，但可以通过 [手动提交 issues](https://github.com/Kenshin/simpread/labels/new%20site) 方式。
- 聚焦模式；  
